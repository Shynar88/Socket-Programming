...
int main(int argc, char *argv[]) {
    ...
    for(;;) {
        ...
        if (!pid) { // in the child    pid == 0
            ...
            //!!!!!!!!!! TODO recv must be put into while loop maybe?
            size_received = recv(client_socket_fd, msg_in, sizeof(struct msg), 0);
            if (size_received == -1) {
            ...
            if (check_checksum((char *) msg_in, (int) ntohll(size_received)) != 0xffff) { //length of msg itself might be better
            ...
            //end of while loop of read

            //reset msg_in msg_out
            }
